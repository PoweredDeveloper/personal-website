...
export interface AnimationOptions {
  /** Enable reveal animation (default: true). If false, shows final result immediately */
  animated?: boolean
...
const DEFAULT_CHARSET = ' .\'`^",:;Il!i~+_-?][}{1)(|/tfjrxnuvczXYUJCLQ0OZmwqpdbkhao*#MW&8%B@$'.split('').reverse().join('')
const SYMBOL_ASPECT_RATIO = 5 / 3 // width/height
...
export default function AsciiImage({ imageUrl, options = {} }: AsciiImageProps) {
  ...
  const currentDisplayRef = useRef<string[][]>([]) // Current displayed state for idle animation
  const brightnessDataRef = useRef<number[][]>([]) // Store brightness for each pixel

  // Load and process image
  useEffect(() => {
    if (!containerRef.current || !imageUrl) return

    // Default reveal speed function (ease-out)
    ...
    // Get a random character with similar brightness to target
    // messinessThreshold: 0 = no mess (correct char), 1 = complete mess (full charset range)
    const getSimilarBrightnessChar = (targetBrightness: number, charSet: string, threshold: number): string => {
      ...
      if (threshold === 0) {
        // No mess - return correct character
        return charSet[targetIndex]
      }

      if (threshold >= 1) {
        // Complete mess - pick from entire charset
        return charSet[Math.floor(Math.random() * charSet.length)]
      }

      // Calculate range based on threshold
      // threshold = 0: range = 0 (no mess)
      // threshold = 0.3: range = 30% of max possible (moderate mess)
      // threshold = 1: full charset (complete mess, handled above)
      const maxRange = charSet.length / 2 // Maximum possible range (half charset each direction)
    ...
    const convertToAscii = (
      ...
      // Draw image to canvas, scaling it to fit
      ...
      for (let y = 0; y < targetRows; y++) {
        ...
        for (let x = 0; x < targetCols; x++) {
          ...
          // Get neighbors for sharpness calculation
          ...
          for (let dy = -1; dy <= 1; dy++) {
            for (let dx = -1; dx <= 1; dx++) {
              ...
              if (nx >= 0 && nx < targetCols && ny >= 0 && ny < targetRows) {
                const ni = (ny * targetCols + nx) * 4
                const nGray = (0.3 * data[ni] + 0.6 * data[ni + 1] + 0.1 * data[ni + 2]) / 255
          ...
          // Store brightness for idle animation
          brightness[y][x] = gray
          ...
          // Create context-aware messy character
      ...
      for (let y = 0; y < asciiDataRef.current.length; y++) {
        for (let x = 0; x < asciiDataRef.current[y].length; x++) {
          ...
          if (useCurrentDisplay && currentDisplayRef.current[y]?.[x]) {
            // Use current display state (for idle animation)
            ...
            // Check if this pixel's random reveal time has passed
            const revealTime = revealTimesRef.current[y]?.[x] ?? 1

            if (revealMode === 'empty') {
              // Empty mode: start with space, reveal correct character
              char = progress >= revealTime ? correctChar : ' '
            } else {
              // Messy mode (default): start with messy, reveal correct character
              char = progress >= revealTime ? correctChar : messyChar
            }

            // Update current display ref
    ...
    // Idle animation - randomly change some symbols to similar ones
    const startIdleAnimation = (charSet: string) => {
      if (!idleAnimation) return

      const changeAmount = idleChangeAmount
      ...
      idleIntervalRef.current = window.setInterval(() => {
        ...
        const pixelsToChange = Math.max(1, Math.floor(totalPixels * changeAmount))
        ...
        for (let y = 0; y < asciiDataRef.current.length; y++) {
          for (let x = 0; x < asciiDataRef.current[y].length; x++) {
            ...
            // Only change non-empty pixels that are not too light (brightness > 0 and < 0.95)
    ...
    const animate = (startTime: number) => {
      ...
      if (progress < 1) {
        ...
        renderAscii(1)
        // Start idle animation after main animation completes
        if (idleAnimation) {
    ...
    img.onload = () => {
      ...
      if (cols !== undefined && rows !== undefined) {
        ...
        // Only cols specified, calculate rows based on image aspect ratio
        targetCols = cols
        targetRows = Math.round(cols / imageAspectRatio / symbolAspectRatio)
      } else if (rows !== undefined) {
        // Only rows specified, calculate cols based on image aspect ratio
        targetRows = rows
        targetCols = Math.round(rows * imageAspectRatio * symbolAspectRatio)
      } else {
        // Neither specified, use image dimensions
      ...
      // Set container and pre element styles first
      if (preRef.current) {
      ...
      // Initialize current display with correct characters
      currentDisplayRef.current = ascii.map((row) => [...row])

      // Generate random reveal times for each pixel (0 to 1)
      revealTimesRef.current = []
      for (let y = 0; y < ascii.length; y++) {
        revealTimesRef.current[y] = []
        for (let x = 0; x < ascii[y].length; x++) {
          // Random value between 0 and 1 for when this pixel should reveal
      ...
      if (ascii.length > 0) {
        if (animated) {
          ...
          // Show final result immediately (static)
          renderAscii(1)
          // Still start idle animation if enabled
          if (idleAnimation) {
            startIdleAnimation(charset)
          }
        }
    ...
    img.onerror = (error) => {
      ...
      if (preRef.current) {
        preRef.current.textContent = 'Failed to load image'
      }
    }
